...
# --- Setup wizard: dispatch early, before heavy imports ---
...
# --- Auto-extract client credentials from client_secret.json ---
# This allows GOOGLE_CLIENT_SECRET_PATH to be the only env var needed.
# Must run before any auth.oauth_config usage (which reads GOOGLE_OAUTH_CLIENT_ID).
_cs_path = os.getenv("GOOGLE_CLIENT_SECRET_PATH") or os.getenv("GOOGLE_CLIENT_SECRETS")
if _cs_path and not os.getenv("GOOGLE_OAUTH_CLIENT_ID"):
    try:
        import json as _json
        with open(_cs_path) as _f:
            _cs = _json.load(_f)
            _inner = _cs.get("web") or _cs.get("installed") or {}
            if _inner.get("client_id"):
                os.environ["GOOGLE_OAUTH_CLIENT_ID"] = _inner["client_id"]
            if _inner.get("client_secret"):
                os.environ["GOOGLE_OAUTH_CLIENT_SECRET"] = _inner["client_secret"]
    except Exception:
        pass  # Will be caught later by check_client_secrets()
...
def main():
    ...
    if args.single_user:
        ...
        # Auto-infer email from stored credentials if not set
        if not os.getenv("USER_GOOGLE_EMAIL"):
            try:
                from auth.google_auth import _find_any_credentials

                _, found_email = _find_any_credentials()
                if found_email:
                    os.environ["USER_GOOGLE_EMAIL"] = found_email
                    safe_print(f"   Auto-detected email: {found_email}")
            except Exception:
                pass  # Will prompt for auth later

        # Update server instructions with the email
        _email = os.getenv("USER_GOOGLE_EMAIL")
        if _email:
            server.instructions = (
                f"Connected Google account: {_email}\n\n"
                f"When using Google Workspace tools, always use `{_email}` as the "
